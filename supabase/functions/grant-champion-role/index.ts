...
Deno.serve(async (req) => {
  ...
  try {
    const { walletAddress, applicationId } = await req.json();
    ...
    const { error } = await admin
      .from("user_roles")
      .upsert({ user_id: prof.id, role: "champion" }, { onConflict: "user_id,role" });
    if (error) throw error;

    if (applicationId) {
      await admin.from("champion_applications")
    ...
    console.error("grant-champion-role error", e);
